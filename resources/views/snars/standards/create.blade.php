...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} - Tambah Standar SNARS</title>
            ...
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-2 text-sm text-gray-500">
                    ...
                    @if(request()->has('chapter_id'))
                        @php
                            $chapter = \App\Models\SnarsChapter::find(request('chapter_id'));
                        @endphp
                        @if($chapter)
                            <span class="mx-1">/</span>
                            <a href="{{ route('snars.groups.show', $chapter->group) }}" class="hover:underline">{{ $chapter->group->code }}</a>
                            <span class="mx-1">/</span>
                            <a href="{{ route('snars.chapters.show', $chapter) }}" class="hover:underline">{{ $chapter->code }}</a>
                        @endif
                    @endif
                    <span class="mx-1">/</span>
                    <span class="text-gray-700">Tambah Standar</span>
                </div>
            
                <div class="flex justify-between items-center">
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                        {{ __('Tambah Standar SNARS') }}
                    </h2>
                    <div>
                        @if(request()->has('chapter_id'))
                            @php
                            ...
                            @endphp
                            @if($chapter)
                                <a href="{{ route('snars.chapters.show', $chapter) }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 focus:bg-gray-300 active:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                    {{ __('Kembali ke Bab') }}
                                </a>
                            @endif
                        @else
                ...
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 bg-white border-b border-gray-200">
                            ...
                            <!-- Create Form -->
                            <form method="POST" action="{{ route('snars.standards.store') }}">
                                @csrf
                                @if(request()->has('chapter_id'))
                                    @php
                                        $chapter = \App\Models\SnarsChapter::find(request('chapter_id'));
                                    @endphp
                                    @if($chapter)
                                        <input type="hidden" name="group_id" value="{{ $chapter->group_id }}">
                                    @endif
                                @endif
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <textarea name="purpose" id="purpose" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('purpose') }}</textarea>
                                    </div>

                                    <!-- Urutan -->
                                    <div>
                                        <label for="order" class="block text-sm font-medium text-gray-700">{{ __('Urutan') }}</label>
                                        <input type="number" name="order" id="order" value="{{ old('order') }}" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        <p class="mt-1 text-xs text-gray-500">{{ __('Jika dikosongkan, akan diisi otomatis dengan urutan terakhir.') }}</p>
                                    </div>
                                    ...
                                    <div>
                                        <div class="flex items-start mt-6">
                                            <div class="flex items-center h-5">
                                                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', '1') == '1' ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                            </div>
                                ...
                                <div class="mt-6 flex justify-end">
                                    @if(request()->has('chapter_id'))
                                        ...
                                        @endphp
                                        @if($chapter)
                                            <button type="button" onclick="window.location='{{ route('snars.chapters.show', $chapter) }}'" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mr-2">
                                                {{ __('Batal') }}
                                            </button>
                                        @endif
                                    @else
                                        <button type="button" onclick="window.location='{{ route('snars.standards.index') }}'" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mr-2">
                                            {{ __('Batal') }}
                                        </button>
                                    @endif
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateGroupId() {
            ...
            fetch(`/api/chapters/${chapterId}`)
                ...
                .then(data => {
                    // Update atau buat hidden input group_id
        ...
        if (chapterSelect) {
            ...
            // Panggil saat halaman dimuat jika sudah ada nilai
