...
<head>
    ...
    <title>{{ config('app.name', 'Laravel') }} - Edit Kelompok SNARS</title>
        ...
        <header class="bg-white shadow">
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-2 text-sm text-gray-500">
                    ...
                    <span class="mx-1">/</span>
                    <a href="{{ route('snars.groups.show', $group) }}" class="hover:underline">{{ $group->code }}</a>
                    <span class="mx-1">/</span>
                ...
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 bg-white border-b border-gray-200">
                            <form method="POST" action="{{ route('snars.groups.update', $group) }}">
                                @csrf
                                @method('PUT')
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <input id="code" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" type="text" name="code" value="{{ old('code', $group->code) }}" required autofocus />
                                        ...
                                        <div>
                                            ...
                                            <input id="name" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" type="text" name="name" value="{{ old('name', $group->name) }}" required />
                                        </div>

                                        <!-- Order -->
                                        <div>
                                            <label for="order" class="block font-medium text-sm text-gray-700">{{ __('Urutan') }}</label>
                                            <input id="order" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" type="number" name="order" value="{{ old('order', $group->order) }}" min="1" />
                                            <p class="mt-1 text-xs text-gray-500">{{ __('Urutan tampilan kelompok.') }}</p>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select id="is_active" name="is_active" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                                <option value="1" {{ old('is_active', $group->is_active) ? 'selected' : '' }}>{{ __('Aktif') }}</option>
                                                <option value="0" {{ old('is_active', $group->is_active) ? '' : 'selected' }}>{{ __('Tidak Aktif') }}</option>
                                            </select>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <select id="version_id" name="version_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                                @foreach($versions as $version)
                                                    <option value="{{ $version->id }}" {{ old('version_id', $group->version_id) == $version->id ? 'selected' : '' }}>
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <textarea id="description" name="description" rows="3" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('description', $group->description) }}</textarea>
                                    </div>
                                </div>
